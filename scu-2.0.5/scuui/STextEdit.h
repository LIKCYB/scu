namespace scuui{
    ...
    public:
        ...
        STextEdit(unsigned int X,unsigned int Y,unsigned int Width,unsigned int Height,const char* Text){
            (*_cId) = scuui::scusystem::AllControlIds;scuui::scusystem::AllControlIds += 1;
            scuui::scusystem::Controls.insert(std::pair<int,scuui::scusystem::Control>((*_cId),(*(new scuui::scusystem::Control))));
            scuui::scusystem::Controls[*_cId].Caption = const_cast<char*>(Text);
        ...
        bool PutOnMainWindow(){//在SMainWindow类AddControl()调用，不需要调用此函数。
            scuui::scusystem::Controls[*_cId].handle = CreateWindowEx(0,TEXT("EDIT"),scuui::scusystem::Controls[*_cId].Caption,WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL | ES_LEFT,
        ...
        char* Text(int MaxDataSize){
            char* _rV = (char*)malloc(sizeof(char) * MaxDataSize);
            GetWindowText(scuui::scusystem::Controls[*_cId].handle,_rV,MaxDataSize);
            return _rV;
        }
        bool Text(const char* NewText){
            return (SetWindowText(scuui::scusystem::Controls[*_cId].handle,const_cast<char*>(NewText)) == TRUE);
        }
